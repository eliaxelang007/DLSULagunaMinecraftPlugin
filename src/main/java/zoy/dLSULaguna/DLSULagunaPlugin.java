...
public final class DLSULagunaPlugin extends JavaPlugin {
    public DLSULaguna dlsuLaguna;

    @Override
    public void onEnable() {
        /*
         * It's possible to forget to initialize a field in [onEnable].
         * That's why I moved the initialization logic to the constructor of
         * [DLSULaguna] so that the java compiler helps us if we forget to
         * initialize something.
         */
        dlsuLaguna = new DLSULaguna(this);
        dlsuLaguna.enable();
    }
...
// getCommand("duelaccept").setExecutor((s, c, l, a) ->
// duels.duelAcceptCommand(s, c, l, a));
// getCommand("dueldeny").setExecutor((s, c, l, a) -> duels.duelDenyCommand(s,
// c, l, a));
...
// new PlayerChatListener(this, (SectionChat)
// getCommand("sectionchat").getExecutor()),
// this);
// Bukkit.getPluginManager().registerEvents(new PlayerStatTracker(this), this);
// Bukkit.getPluginManager().registerEvents(new PlayerSectionListener(this),
// this);
